...
4. **Source discovery:** load `content/rules/mcp-first-search.md` before searching 1C sources. Follow its bounded chain within verified contour coverage, including applicable `grep=true` retry; explain native fallback once. No eligible exposed index → scoped native search immediately. Check freshness before claiming absence. Direct reads of edit targets are allowed.
